...
save $dta/retreffdistances.dta , replace


*********************
* 2) retreffdistance in ab and sk
clear
use $dta/ab_sk.dta 
...
*	qui	replace El2 = ElementID[_n+(`i'*87)+87] //if _n>`a'
	...
	qui keep if _n<=`t'
	gen simLevel = 1 																// changed
	gen simImp = 1 																	// changed
	qui replace simLevel = normalden(5*(hLevel-hLev2))/normalden(0) if hLevel<hLev2	// changed
	qui replace simImp = normalden(5*(hImp-hImp2))/normalden(0) if hImp<hImp2		// changed
	gen D1_d_skills = (1-(simImp*simLevel))
	// qui replace  D1_d_skills = . if hImp<=0 & hImp2<=0  							// dropped	
	collapse (mean) D1_d_skills=D1_d_skills , by(Occupation occ2)					// changed
...
*	qui	replace El2 = ElementID[_n+(`i'*33)+33] //if _n>`a'
	...
	// qui replace  D2_d_know = . if hImp<=0 & hImp2<=0 							// dropped	
	collapse (mean) D2_d_know=D2_d_know , by(Occupation occ2) 						// changed
